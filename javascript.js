...
function KubAdd(col, newW) {
  ...
  for (let i = 0; i < col; i++) {
    ...
    $("#games").append(
      '<div class="kybik" data-color="' +
        color +
        '" onClick="KubClick(this)" style="width: ' +
        newW +
        "px; height: " +
        newW +
        "px; top: " +
...
function KubClick($this) {
  ...
  if (color == "yellow") points = +points + 2;
  if (width < 20) points = +points + 2;
  else points++;
  $("#points").text(points);
  $($this).remove();
  KubAdd(getRandomArbitrary(0, 2));

  if (document.getElementById("games").childNodes.length < 1) KubAdd(3);
}
